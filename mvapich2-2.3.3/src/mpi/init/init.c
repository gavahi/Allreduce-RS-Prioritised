...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if ((s_value = getenv("MV2_RANK_TEST")) != NULL) {
        rank_test = (atoi(s_value));                
    }

    
    if ((s_value = getenv("MV2_PRO_RED_SCT")) != NULL) {
        pro_red_sct = (atoi(s_value));                
    }
	
	
    if ((s_value = getenv("MV2_PRINT_FUN_NAME")) != NULL) {
        print_fun_name = (atoi(s_value));                
    }
