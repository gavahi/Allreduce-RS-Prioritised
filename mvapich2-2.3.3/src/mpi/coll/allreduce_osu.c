                                ...
                                MPI_Op op, MPID_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (newrank != -1)
    {
        ...
        { /* use recursive doubling */
            mask = 0x1;
            if (print_fun_name == 1 && comm_ptr->rank == 0)
                fprintf(stderr, "Recursive_doubling\n");
            while (mask < pof2)
            {
                newdst = newrank ^ mask;
                /* find real rank of dest */
        ...
        else
        {
            ...
            if (pro_red_sct == 1)
            {
                ...
                if (print_fun_name == 1 && comm_ptr->rank == 0)
                    fprintf(stderr, "Pro_reduce_scatter_allgather\n");

                MPID_Request **send_req, **recv_req;
                MPI_Status *send_st, *recv_st;
                MPIU_CHKLMEM_MALLOC(send_req, MPID_Request **, comm_size * sizeof(MPID_Request *),
                                    mpi_errno, "send_req");
                MPIU_CHKLMEM_MALLOC(recv_req, MPID_Request **, comm_size * sizeof(MPID_Request *),
                                    mpi_errno, "recv_req");

                MPIU_CHKLMEM_MALLOC(send_st, MPI_Status *, comm_size * sizeof(MPI_Status),
                                    mpi_errno, "send_st");
                MPIU_CHKLMEM_MALLOC(recv_st, MPI_Status *, comm_size * sizeof(MPI_Status),
                                    mpi_errno, "recv_st");
                ...
                int nprocs, datatype_sz, len, temp_rank, basic_seg_size;
                    ...
                    for (i = 0; i < (Max_pow / lvl_pow); i++)
                    {
                        ...
                        if (lvl > 1)
                        {
                            ...
                            for (k = 2; k <= lvl; k++)
                            {
                                ...
                                // if (print_fun_name == 1)   fprintf(stderr, "[%d] k=%d/%d  pkg_curr=%d 1\n", rank,k,lvl,pkg_curr);

                                mpi_errno = MPIC_Waitall(1, &recv_req[pkg_curr], &recv_st[pkg_curr], errflag);

                                // if (print_fun_name == 1)  fprintf(stderr, "[%d] k=%d/%d   2\n", rank,k,lvl);

                                if (mpi_errno)
                                    MPIR_ERR_POP(mpi_errno);

                                (*uop)((char *)(tmp_buf + (last_oprd * seg_size + jump) * extent),
                                       (char *)(recvbuf + indx * seg_size * extent),
                                       &seg_size, &datatype);

                                // if (print_fun_name == 1)  fprintf(stderr, "[%d] k=%d/%d   3\n", rank,k,lvl);

                                if (k == 2)
                                    base_jump = (comm_size / 4);
                                else
                                    base_jump = base_jump / 2;
                                jump += seg_size * base_jump;
                                pkg_curr += base_jump;
                            }
                        ...
                        // if (print_fun_name == 1) fprintf(stderr, "[%d] 3-1\n",rank);

                        /* Send data from recvbuf. Recv into tmp_buf */
                        MPIR_PVAR_INC(allreduce, pt2pt_rs, send, send_cnt, datatype);
                        MPIR_PVAR_INC(allreduce, pt2pt_rs, recv, recv_cnt, datatype);

                        // if (print_fun_name == 1) fprintf(stderr, "[%d] 4\n",rank);

                        mpi_errno = MPIC_Irecv((char *)tmp_buf + next_free * seg_size * extent,
                                               seg_size, datatype, MPIR_Rank_list_mapper(comm_ptr, dst),
                                               MPIR_ALLREDUCE_TAG, comm_ptr, &recv_req[pkg_num]);

                        // if (print_fun_name == 1) fprintf(stderr, "[%d] 5 pkg_num=%d\n",rank,pkg_num); 

                        mpi_errno = MPIC_Isend((char *)recvbuf + indx * seg_size * extent,
                                               seg_size, datatype, MPIR_Rank_list_mapper(comm_ptr, dst),
                                               MPIR_ALLREDUCE_TAG, comm_ptr, &send_req[pkg_num++], errflag);
                        // if (print_fun_name == 1) fprintf(stderr, "[%d] 6  pkg_num=%d\n",rank,pkg_num);
                ...
                // if (print_fun_name == 1) fprintf(stderr, "[%d] 7\n",rank);
                ...
                for (k = 2; k <= lvl; k++)
                {
                    ...
                    // fprintf(stderr, "[%d] k=%d/%d  pkg_curr=%d 8\n", rank,k,lvl,pkg_curr);

                    mpi_errno = MPIC_Waitall(1, &recv_req[pkg_curr], &recv_st[pkg_curr], errflag);

                    // fprintf(stderr, "[%d] k=%d/%d  pkg_curr=%d 9\n", rank,k,lvl,pkg_curr);
                    
                    (*uop)((char *)(tmp_buf + (last_oprd * seg_size + jump) * extent),
                           (char *)(recvbuf + indx * seg_size * extent),
                           &seg_size, &datatype);

                    // (stderr, "[%d] k=%d/%d  pkg_curr=%d 10\n", rank,k,lvl,pkg_curr);
                    if (k == 2)
                        base_jump = (comm_size / 4);
                    else
                        base_jump = base_jump / 2;
                ...
                // fprintf(stderr, "[%d] End 11---\n", rank);
            }
            else
            {

                if (print_fun_name == 1 && comm_ptr->rank == 0)
                    fprintf(stderr, "Normal_reduce_scatter_allgather\n");
                           ...
                           MPI_Op op, MPID_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
...
#endif /* MPID_HAS_HETERO */
    {
        ...
        {
            while ((range_threshold < (mv2_allreduce_thresholds_table[range].size_inter_table - 1)) && ((mv2_allreduce_thresholds_table[range].inter_leader[range_threshold].MV2_pt_Allreduce_function == &MPIR_Allreduce_mcst_reduce_redscat_gather_MV2) ||
                                                                                                        (mv2_allreduce_thresholds_table[range].inter_leader[range_threshold].MV2_pt_Allreduce_function == &MPIR_Allreduce_mcst_reduce_two_level_helper_MV2)))
            {
                range_threshold++;
            }
        }
        while ((range_threshold < (mv2_allreduce_thresholds_table[range].size_inter_table - 1)) && (nbytes > mv2_allreduce_thresholds_table[range].inter_leader[range_threshold].max) && (mv2_allreduce_thresholds_table[range].inter_leader[range_threshold].max != -1))
        {
            range_threshold++;
        }
        ...
        MV2_Allreduce_function = mv2_allreduce_thresholds_table[range].inter_leader[range_threshold].MV2_pt_Allreduce_function;

        MV2_Allreduce_intra_function = mv2_allreduce_thresholds_table[range].intra_node[range_threshold_intra].MV2_pt_Allreduce_function;
        ...
        if (is_two_level == 1)
        {
...
#endif /* #if defined(_MCST_SUPPORT_) */
            {
                ...
                if ((comm_ptr->dev.ch.shmem_coll_ok == 1) && (mv2_enable_shmem_allreduce) && (is_commutative) && (mv2_enable_shmem_collectives))
                {
                                         ...
                                         MPI_Op op, MPID_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
...
#if defined(_SHARP_SUPPORT_)
    int is_socket_aware = mv2_enable_socket_aware_collectives && mv2_use_socket_aware_allreduce && mv2_use_socket_aware_sharp_allreduce;

    // If is_socket_aware == 1 then Sharp allreduce will be called from within the socket-aware
    // allreduce function
    if (comm_ptr->dev.ch.is_sharp_ok == 1 && nbytes <= mv2_sharp_tuned_msg_size && mv2_enable_sharp_coll == 2 && !is_socket_aware)
    {
    ...
    if (comm_ptr->dev.ch.shmem_coll_ok == 1 && comm_ptr->dev.ch.is_uniform)
    {
        ...
        if (nbytes <= mv2_socket_aware_allreduce_max_msg && nbytes >= mv2_socket_aware_allreduce_min_msg && mv2_enable_socket_aware_collectives && mv2_use_socket_aware_allreduce && comm_ptr->dev.ch.shmem_coll_ok == 1 && comm_ptr->dev.ch.allreduce_coll_count >= shmem_coll_count_threshold && comm_ptr->dev.ch.use_intra_sock_comm == 1 && is_commutative)
        {
...
conf_check_end:
    ...
    */
...
#endif /* MPID_HAS_HETERO */
    {
        ...
        table_max_comm_size =
            mv2_allreduce_indexed_thresholds_table[conf_index][mv2_size_allreduce_indexed_tuning_table[conf_index] - 1].numproc;
        ...
        table_max_inter_size = mv2_allreduce_indexed_thresholds_table[conf_index][comm_size_index].inter_leader[last_inter].msg_sz;
        ...
        table_max_intra_size = mv2_allreduce_indexed_thresholds_table[conf_index][comm_size_index].intra_node[last_intra].msg_sz;
        ...
        if (mv2_allreduce_indexed_thresholds_table[conf_index][comm_size_index].is_two_level_allreduce[inter_node_algo_index] == 1)
        {
            is_two_level = 1;
        }

        MV2_Allreduce_function = mv2_allreduce_indexed_thresholds_table[conf_index][comm_size_index].inter_leader[inter_node_algo_index].MV2_pt_Allreduce_function;

        MV2_Allreduce_intra_function = mv2_allreduce_indexed_thresholds_table[conf_index][comm_size_index].intra_node[intra_node_algo_index].MV2_pt_Allreduce_function;
...
#if defined(_SHARP_SUPPORT_)
        if (comm_ptr->dev.ch.is_sharp_ok == 1 && nbytes <= mv2_sharp_tuned_msg_size && mv2_enable_sharp_coll == 1)
        {
            is_two_level = 1;
...
#if defined(_MCST_SUPPORT_)
    if (comm_ptr->dev.ch.is_mcast_ok == 1 && comm_ptr->dev.ch.shmem_coll_ok == 1 && mv2_use_mcast_allreduce == 1 && stride >= mv2_mcast_allreduce_small_msg_size && stride <= mv2_mcast_allreduce_large_msg_size)
    {
...
#endif /* #if defined(_MCST_SUPPORT_) */
    {
        if ((comm_ptr->dev.ch.shmem_coll_ok == 1) && (stride < mv2_coll_param.allreduce_2level_threshold) && (mv2_enable_shmem_allreduce) && (is_commutative) && (mv2_enable_shmem_collectives))
        {
                                  ...
                                  MPI_Op op, MPID_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    int context_id = (comm_ptr->comm_kind == MPID_INTRACOMM) ? MPID_CONTEXT_INTRA_COLL : MPID_CONTEXT_INTER_COLL;
